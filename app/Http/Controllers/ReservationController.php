...
class ReservationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $user = Auth::user();
            $facility = Facility::with('fees')->find($validated['facility_id']);

            // SPECIAL HANDLING FOR BASKETBALL COURT (facility_id = 2)
            if ($validated['facility_id'] == 2) {
                // Use time-based fee calculation for basketball court
                $start = Carbon::parse($validated['start_time']);
                $fees = $facility->fees->sortBy('start_time');

                // Find the fee based on start time (same logic as availability2)
                $slotFee = $fees->first(function ($fee) use ($start) {
                    $feeStart = Carbon::parse($fee->start_time);
                    $feeEnd = Carbon::parse($fee->end_time);
                    return $start->between($feeStart, $feeEnd->subMinute());
                });

                $latestFee = $slotFee ?: $fees->first();
            } else {
                // ORIGINAL LOGIC FOR OTHER FACILITIES
